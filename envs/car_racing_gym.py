...
class CarRacing(gym.Env, EzPickle):
    ...
    def __init__(
        ...
        self.obs_mean = np.array([
            0.0,
            8.83e+01,  4.37e+01,  1.06e+00, -1.89e-01,  2.15e+01,  1.77e-01
            # ,  6.50e-02,
            # 6.50e-02,  0.00e+00,  0.00e+00,  3.94e+01,  4.01e+01,  3.98e+01,  4.05e+01,
            # 0.00e+00,  0.00e+00,  9.00e-03,  9.00e-03,  0.00e+00,  0.00e+00,  0.00e+00,
            # 0.00e+00
        ], dtype=np.float32)

        self.obs_std = np.array([
            1.0,
            1.16e+02, 5.26e+01, 8.95e-01, 7.60e-01, 7.47e+00, 6.43e-01
            # , 3.21e-01, 3.21e-01,
            # 0.00e+00, 0.00e+00, 1.40e+01, 1.42e+01, 1.36e+01, 1.38e+01, 0.00e+00, 0.00e+00,
            # 6.90e-02, 6.90e-02, 0.00e+00, 0.00e+00, 0.00e+00, 0.00e+00
        ], dtype=np.float32)
        ...
        if self.obs_mode == "pixels":
            ...
            # Example: [x, y, angle, vx, vy, angular_vel, wheel_steer0..1, wheel_omega0..3, wheel_gas0..3, wheel_brake0..3]
            # dim = 7 + 4 * 4
            # dim = 7
            # dim = 6
            dim = 16
    ...
    def _create_track_deterministic(self):
        CHECKPOINTS = 12

        rng = np.random.RandomState(42)  # fixed RNG, always same output

        checkpoints = []
        for c in range(CHECKPOINTS):
            noise = rng.uniform(0, 2 * math.pi * 1 / CHECKPOINTS)
            alpha = 2 * math.pi * c / CHECKPOINTS + noise
            rad = rng.uniform(TRACK_RAD / 3, TRACK_RAD)
    ...
    def _get_obs_absolute(self):
        if self.obs_mode == "pixels":
            return self._render("state_pixels")
            ...
            v_forward = forward[0] * vx + forward[1] * vy
            v_side = side[0] * vx + side[1] * vy
            ...
            # Add wheel info
            # for w in car.wheels:
            #     state.append(w.steer)
            # for w in car.wheels:
            #     state.append(w.omega)
            # for w in car.wheels:
            #     state.append(w.gas)
            # for w in car.wheels:
            #     state.append(w.brake)
    ...
    def _get_obs(self, lookahead=5):
        hull = self.car.hull
        car_x, car_y = hull.position
        car_angle = hull.angle

        # 1. find nearest track point
        dists = [(tx - car_x) ** 2 + (ty - car_y) ** 2 for _, _, tx, ty in self.track]
        i = int(np.argmin(dists))
        alpha, beta, cx, cy = self.track[i]

        # 2. heading error (wrap to [-pi,pi])
        heading_error = (car_angle - beta + np.pi) % (2 * np.pi) - np.pi

        # 3. lateral error: signed distance from centerline
        dx, dy = car_x - cx, car_y - cy
        perp = (-math.sin(beta), math.cos(beta))  # normal to track tangent
        lateral_error = dx * perp[0] + dy * perp[1]

        # left/right edge positions
        road_l = (cx - TRACK_WIDTH * math.cos(beta),
                  cy - TRACK_WIDTH * math.sin(beta))
        road_r = (cx + TRACK_WIDTH * math.cos(beta),
                  cy + TRACK_WIDTH * math.sin(beta))

        # distance from car to left/right edges (projected onto perp vector)
        dist_left = (car_x - road_l[0]) * perp[0] + (car_y - road_l[1]) * perp[1]
        dist_right = (car_x - road_r[0]) * perp[0] + (car_y - road_r[1]) * perp[1]

        # 4. velocities in car frame
        vx, vy = hull.linearVelocity
        forward = (math.cos(car_angle), math.sin(car_angle))
        side = (-forward[1], forward[0])
        v_forward = vx * forward[0] + vy * forward[1]
        v_side = vx * side[0] + vy * side[1]
        omega = hull.angularVelocity

        # 5. lookahead waypoints relative to car
        waypoints = []
        for j in range(1, lookahead + 1):
            _, _, wx, wy = self.track[(i + j) % len(self.track)]
            dx, dy = wx - car_x, wy - car_y
            # rotate into car frame
            rel_x = math.cos(-car_angle) * dx - math.sin(-car_angle) * dy
            rel_y = math.sin(-car_angle) * dx + math.cos(-car_angle) * dy
            waypoints += [rel_x, rel_y]

        obs = np.array([dist_left, dist_right, heading_error,
                        v_forward, v_side, omega] + waypoints,
                       dtype=np.float32)
        return obs
    ...
    def reset(
        ...
        *,
        ...
        while True:
            # success = self._create_track_random()
            success = self._create_track_deterministic()
            if success:
                break
            if self.verbose:
                print(
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        step_reward = 0
        terminated = False
        truncated = self.ep_len >= self.max_ep_len
    ...
    def _render(self, mode: str):
        ...
        forward = (math.cos(self.car.hull.angle), math.sin(self.car.hull.angle))
        side = (-forward[1], forward[0])
        v_forward = vx * forward[0] + vy * forward[1]
        v_side = vx * side[0] + vy * side[1]

        font = pygame.font.Font(pygame.font.get_default_font(), 42)
        text = font.render(f"vel: {v_side:.2f}, score: {self.reward:.2f}",True, (255, 255, 255), (0, 0, 0))
        text_rect = text.get_rect()
        text_rect.center = (WINDOW_W / 2, WINDOW_H - WINDOW_H * 2.5 / 20.0)
